...
Let \( | G  |  =  n \) and suppose \( G  \) is a cyclic group. Then we know that there is some element \( x  \) whose order is \( n  \). Now, suppose \( a \in \Z - \{ 0  \}  \), then we have further that 
\[  | x^{a} |  = \frac{ n  }{ (n,a) }  \]
and so the cyclic subgroup generated by \( x^{a} \) has order \( \frac{ n  }{ (n,a) }  \).Consider the following statement: If \( | G  |  = n < \infty   \), then for each positive integer \( a  \), there is a unique subgroup of \( G  \) of order \( a \). This subgroup is the cyclic group \( \langle  x^{d} \rangle  \) where \( d = \frac{ n }{ a }  \). 

Prove that if \( | G  |  = 231 = 3 \cdot 7 \cdot 11  \), then \( | Z (G) |  \geq 11  \).
