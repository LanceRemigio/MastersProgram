...
Since \( H  \) is normal, (that is, \( {g}_{2}' H = H {g}_{2}' \) and \( {g}_{1}' = H {g}_{1}' \))
\begin{align*}
...
\begin{remark}[Observation]
   Suppose \( G  \) is a Lie group and \( N \) is a normal subgroup algebraic subgroup of \( G  \). From this we can construct a new algebraic group \( G / N   \). In the context of Lie Theory, an important question arises: 
   \begin{center}
...
What can we conclude from this result? If we manage to find a smooth structure on \( G / N  \) that ensures \( \pi : G \to G / N   \) is smooth, then \( N  \) is necessarily a topologically closed subset of \( G  \) (recall that if \( \pi  \) is smooth, then it is continuous and preimages of continuous functions of closed sets are closed). If it is not closed, then there is no way to define a smooth structure on the homomorphism \( \pi \).
