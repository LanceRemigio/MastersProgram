       ...
       \item[(ii)] Prove that \( f  \) is continuous if and only if \( f(\overline{A})  \subseteq \overline{f(A)}   \) for all \( A \subseteq  X  \).
           \begin{proof}
               ...
               Note that \( \overline{f(A)} \) is a closed set in \( Y  \) and so its preimage under \( f  \), \( f^{-1}(\overline{f(A)}) \), is closed in \( X  \) from the previous part. Since \( f(A) \subseteq \overline{f(A)} \), we have that \( A \subseteq f^{-1}(\overline{f(A)}) \). Since \( f^{-1}(\overline{f(A)})\) is a closed set in \( X  \) containing \( A  \) and \( \overline{A} \) is the smallest closed set in \( X  \) containing \( A  \), we have that \( \overline{A} \subseteq f^{-1}(\overline{f(A)}) \), meaning that \( f(\overline{A}) \subseteq  \overline{f(A)} \).
                ...
                \end{align*}
                Thus, we conclude that \( f^{-1}(C) \) is closed in \( X  \).
           \end{proof}
       \item[(iii)] Prove that if, \( f  \) is continuous and \( A  \) is dense in \( X  \), then \( f(A) \) is dense in \( f(X) \).
           \begin{proof}
           Our goal is to show that \( \overline{f(A)} = f(X) \); that is, we want to show that \( f(X) \subseteq  \overline{f(A)} \) and \( \overline{f(A)} \subseteq f(X) \). Starting with the first inclusion, we observe that since \( f  \) is continuous and \( A  \) is dense in \( X  \) (i.e \( \overline{A} = X  \)), it follows from part (ii) that 
           \[  f(X) = f(\overline{A}) \subseteq \overline{f(A)} \implies f(X) \subseteq \overline{f(A)}. \tag{1} \]
           Now, we need to show that \( \overline{f(A)} \subseteq f(X) \). Since \( f^{-1}(\overline{f(A)}) \subseteq X  \). Taking the image under \( f \), it follows that \( \overline{f(A)} \subseteq f(X) \) (2). 

           From (1) and (2), it follows that \( \overline{f(A)} = f(X)  \).
           \end{proof}
   \end{enumerate}
\end{problem}
...
\begin{problem}[Continuity and Lower Limit Topology]
   \begin{enumerate}
       \item[(i)] Let \( {\mathcal{T}}_{L} \) be the lower limit topology and \( {\mathcal{T}}_{E} \) be the Euclidean topology on \( \R  \). Prove that \( f: (\R , {\mathcal{T}}_{L}) \to (\R , {\mathcal{T}}_{E}) \) is continuous if and only if for every \( x \in \R  \) and \( \epsilon > 0  \), there exists \( \delta > 0  \) such that \( | f(x) - f(y) |  < \epsilon  \) for all \( y \in [x,x+\delta) \).
           \begin{proof}
               ...
               Let \( x \in \R  \) and \( \epsilon > 0  \) be given. Now, consider the open ball \( {B}_{\epsilon}(f(x)) \) and notice that \( x \in f^{-1}({B}_{\epsilon}(f(x))) \). Since \( {B}_{\epsilon}(f(x)) \) is an open set in \( (\R, {\mathcal{T}}_{E}) \) and \( f  \) is continuous, we know that \( f^{-1}({B}_{\epsilon}(f(x))) \) is open in \( (\R , {\mathcal{T}}_{L}) \). Since \( x \in f^{-1}({B}_{\epsilon}(f(x)))  \), there exists an open set \( U' \in {\mathcal{T}}_{L} \) containing \( x  \) such that \( x \in U' \subseteq  f^{-1}({B}_{\epsilon}(f(x))) \). Since \( U' \in {\mathcal{T}}_{L} \), we know that \( U' = [a,b) \) for some \( a,b \in \R \). Since \( x \in U' \), define \( \delta = \frac{ 1 }{ 2 }  (b - x) \) (this is strictly positive because \( x \in [a,b) \); that is, \( a \leq x < b \implies b - x > 0  \) ). Thus, we see that \( [x, x + \delta) \subseteq U' \subseteq f^{-1}({B}_{\epsilon}(f(x)) \) and so we conclude that  
               \[  [x,x+\delta) \subseteq f^{-1}({B}_{\epsilon}(f(x)). \]
               ...
               \end{center}
               Since \( [x,x+ \delta) \in {\mathcal{T}}_{L} \) (\( [x, x + \delta) \) is open with respect to the Lower Limit Topology) containing \( x  \), it follows that 
       ...
       \begin{solution}
           We claim that \( f  \) is continuous. Notice that \( f  \) is continuous for all \( x \in \R \) that is non-zero. It suffices to check that \( f  \) is continuous at \( 0 \in \R   \). We can see this because for all \( \epsilon > 0 \), we can define \( \delta  = 1  \) such that \( [0,1) \subseteq f^{-1}({B}_{\epsilon}(f(0)) = f^{-1}({B}_{\epsilon}(1)) \). That is, \( f  \) is continuous at \( 0  \).
       \end{solution}
   ...
   \end{cases}  \) \ continuous?
   \begin{solution}
       No, because there does not exists an open set \( U \) in \( {\mathcal{T}}_{L} \) such that under the preimage of \( f  \), \( f^{-1}(U) \) is not open in \( \mathcal{T}_{E} \). For example, take \( U = [0,\epsilon) \) for any \( \epsilon > 0  \) in \( {\mathcal{T}}_{L} \). Then \( f^{-1}(U) \) is not open because \( [0,\epsilon) \) is not open with respect to the Euclidean topology. 
   \end{solution}
   \end{enumerate} 
\end{problem}
...
\end{problem}
\begin{proof}
    Let \( U  \) be closed in \( Y  \) and let \( \{ {A}_{i} \}_{i \in I} \) be a finite cover of \( X  \). Our goal is to show that \( f^{-1}(U) \) is open in \( X  \). Notice that 
\begin{align*}
...
Since each \( {f}_{i} \) is continuous on \( {A}_{i} \) for all \( 1 \leq i \leq n  \), and \( U  \) is a closed set in \( Y  \), it follows that \( f^{-1}_i(U) \) is closed for all \( 1 \leq i \leq n  \). Since a finite union of closed sets is closed, it follows that 
\[  f^{-1}(U) = \bigcup_{ i= 1  }^{ n }  f^{-1}_i(U) \]
is a closed set and so we conclude that \( f \) is continuous.
\end{proof}
...
\begin{problem}
   \begin{enumerate}
       \item[(i)] Let \( X  \) be a topological space and \( A  \) is a subspace of \( X  \). Consider the inclusion \( i : A \to X  \), \( i(x) = x  \). Then
           \begin{enumerate}
               \item[(a)] \( A  \) is open in \( X \) if and only if the inclusion \( i \) is open.
                   \begin{proof}
                       Let \( \mathcal{T} \) be a topological space on \( X  \) and let \( \mathcal{T} |_{A} \) be the subspace topology endowed from \( X  \). 
                       \( (\Longrightarrow) \) Suppose \( A  \) is open. Our goal is to show that the inclusion map \( i  \) is open; that is, \( i(U) \in \mathcal{T} \) for all \( U \in \mathcal{T} |_{A}  \). Let \( U \in \mathcal{T} |_{A} \) be arbitrary (that is, \( U  \) is open in \( A  \)). We want to show that \( i(U)  \) is open in \( X  \). To this end, let \( y \in i(U) \) be arbitrary. Then there exists an \( x \in U  \) such that \( i (x) = y  \). But \( U  \) is open in \( A  \), so there exists an open set \( V  \) in \( X  \) such that \( U = V \cap A  \). But \( A  \) is open in \( X  \), so we have that \( U \) is open in \( X  \). Since \( x \in U  \) and \( U  \) is open in \( X  \), there exists an open set \( W  \) in \( X  \) containing \( x  \) such that \( x \in W \subseteq U \). Since \( i(x) = y  \), by definition of the inclusion map, \( i(x)  = x \) and so \( x =y  \). Now, note that \( W \subseteq U  \) implies that \( i(W) \subseteq i(U) \) since \( x \in W  \). However, \( i(W) = W  \) since \( W  \) is open in \( X  \), and so we get that    
                       \[  i(x) \in i(W) \subseteq i(U) \implies y \in W \subseteq i(U). \]
                       This tells us that \( i(U) \) is open in \( X  \) and so \( i  \) is open. 
                ...
                \item[(b)] \( A  \) is closed in \( X  \) if and only if the inclusion \( i \) is closed.
                    \begin{proof}
                        \( (\Longrightarrow) \) Suppose \( A  \) is closed in \( X  \). Our goal is to show that \(  i  \) is closed; that is, we need to show that for all closed sets \( V \) in \( A  \), \( i(V) \) is closed in \( X  \). To this end, let \( V  \) be a closed set in \( A  \). Then there exists an closed set \( U  \) in \( X  \) such that \( V = U \cap A  \) is closed in \( X  \) (since the finite intersection of two closed sets is closed). This tells us that \( V^{c}  \) is open in \( X  \). Set \( W = V^{c} \). Then \( i \) is open from part (a); that is, for all \( B \in \mathcal{T} |_{W} \), \( i(B) \in \mathcal{T} \). Since \( W \in \mathcal{T} |_{W} \), it follows that \( i(W) \in \mathcal{T} \). Then \( i(V^{c}) \in \mathcal{T} \) and so \( [i(V)]^{c} \in \mathcal{T} \). That is, \( i(V) \) is closed in \( X  \).
       ...
       \item[(ii)] Let \( n,k \in \N  \). Prove that the function
           ...
           is not open.
           \begin{proof}
               Construct the open ball \( {B}_{\epsilon}(0) \) centered at \( 0  \) with an arbitrary radius \( \epsilon  \) in \( \R^{n} \). The image of this ball under \( f  \) is \[  f({B}_{\epsilon}(0)) = \{ ({x}_{1}, {x}_{2}, \dots, {x}_{n}, 0, 0, \dots , 0 ) : x \in {B}_{\epsilon}(0) \}. \] 
               Note that there does not exists an open ball in \( \R^{n+k} \) containing \( 0  \) that is completely contained in the above image. We can see this by taking any point with \( k  \)-th component that is non-zero. It will be contained in the open ball, but not in the image because one of the \( k \)th components past \( n  \) is non-zero. Hence, \( f  \) is not an open map. 
           \end{proof}
   \end{enumerate} 
\end{problem}
